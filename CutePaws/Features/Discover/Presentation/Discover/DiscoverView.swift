...
private struct BreedExploreListRoute: Hashable {}

private struct BreedExploreGalleryRoute: Hashable {
...
struct DiscoverView: View {
    ...
    var body: some View {
        NavigationStack(path: $explorePath) {
            ...
            .navigationDestination(for: BreedExploreListRoute.self) { _ in
                ExploreBreedView(
                    breedGalleryRepository: viewModel.breedGalleryRepository,
                    onSelectBreed: { explorePath.append(BreedExploreGalleryRoute(breedName: $0)) }
                )
            }
            .navigationDestination(for: BreedExploreGalleryRoute.self) { route in
                BreedImagesView(
                    breedName: route.breedName,
                    breedGalleryRepository: viewModel.breedGalleryRepository,
                    favoriteRepository: viewModel.favoriteRepository,
                    onRequestImageDetail: { exploreImageDetailViewModel = $0 }
                )
            }
        }
        ...
        .fullScreenCover(item: $exploreImageDetailViewModel, onDismiss: { exploreImageDetailViewModel = nil }) { imageDetailViewModel in
            ImageDetailView(viewModel: imageDetailViewModel)
                .id(imageDetailViewModel.id)
        }
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    DiscoverMiniMomentsSectionView(
                        ...
                        showsPreviewCaption: viewModel.showsMiniMomentsPreviewCaption,
