...
struct DetailImagePagerView: View {
    ...
    var body: some View {
        ...
        .simultaneousGesture(
            DetailPullDownDismiss.gesture(isDismissBlocked: { isCurrentImageZoomed }, onDismiss: onDismiss)
        )
    }
...
/// Pull-down to dismiss used by `DetailImagePagerView` and the empty `FavoritesView` state.
enum DetailPullDownDismiss {
    static let minimumDragDistance: CGFloat = 20
    static let minimumDownwardTranslation: CGFloat = 120

    static func gesture(
        isDismissBlocked: @escaping () -> Bool = { false },
        onDismiss: @escaping () -> Void
    ) -> some Gesture {
        DragGesture(minimumDistance: minimumDragDistance)
            .onEnded { value in
                guard !isDismissBlocked() else { return }
                guard value.translation.height > minimumDownwardTranslation else { return }
...
private struct DetailAspectFittedVideoPage: View {
    ...
    var body: some View {
        ...
        .task(id: url.path) {
            if let r = await VideoAspectRatioReader.videoAspectRatio(fileURL: url) {
                aspectRatio = r
            }
        }
    }
